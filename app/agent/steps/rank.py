...
from app.db.queries import list_week_summaries, upsert_run
...
def valid_picks(out: RankOut, allowed: set[str]) -> list[tuple[str, str]]:
    seen: set[str] = set()
...
def run_rank(
    ...
    summaries = list_week_summaries(settings.window_start())
